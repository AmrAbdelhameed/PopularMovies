...
public class FavouriteGridViewAdapter extends ArrayAdapter<MainResponse.ResultsBean> {

    private Context mContext;
    private int layoutResourceId;
    private List<MainResponse.ResultsBean> mGridData;

    public FavouriteGridViewAdapter(Context mContext, int layoutResourceId, List<MainResponse.ResultsBean> mGridData) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Picasso.with(mContext).load(mGridData.get(position).getPoster_path()).into(holder.Image1);
        return row;
    }
